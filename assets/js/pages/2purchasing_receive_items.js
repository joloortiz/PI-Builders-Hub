Ext.onReady(function() {
	...
	var receiving_store = new Ext.data.Store({
	    fields: ['i_id', 'i_name', 'i_attribute', 'i_purchase_price', 'u_slug_name', 'c_name', 'qty', 'cost', 'pod_id', 's_id', 's_name']
	});
									...
									if(decode.success == false) {
										...
										Ext.Array.each(items, function(item_data, index) {
											receiving_html_items += '\
												<tr>\
													<td style="padding: 0px; line-height: 15px;">' + item_data.qty + '</td>\
	...
	function Update_qty_purchase_price() {
		...
		record.set('s_id', Ext.getCmp('frm_sel_supplier').getValue());
		record.set('s_name', Ext.getCmp('frm_sel_supplier').getRawValue());
