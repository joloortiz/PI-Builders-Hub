...
class Pos_order_taking extends MY_Controller {
	...
	function save() {
		...
		$delivered_to = mysql_real_escape_string($this->input->post('delivered_to'));
		...
		try {
			...
			// If Amount Tendered is less than Total Amount, add credit record
			if($amount_tendered < $total) {
				// Insert Credit Data
				$insert_credit_data = array(
					'cr_amount' => $total - $amount_tendered,
					'o_id' => $o_id
				);
				$this->orders_model->Insert_credit($insert_credit_data);
			}
